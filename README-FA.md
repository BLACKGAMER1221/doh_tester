...
اتصال به پلتفرم‌های فیلترشده مانند YouTube، Instagram، Twitter/X و سایت‌های خبری با حل دامنه‌ها از طریق اتصال HTTPS رمزنگاری‌شده و عبور از فیلترینگ مبتنی بر DNS و ربایش DNS.
...
| وضعیت       | معنی                                                                                       |
| ----------- | ------------------------------------------------------------------------------------------ |
| **WORKING** | نقطه انتهایی تمام تست‌ها را گذرانده است (TCP، TLS و تعداد کوئری‌های موفق ≥ min_success)    |
...
1. **تنظیم تعداد نخ‌ها بر اساس اتصال شما**:

   * کند/ناپایدار: `workers 5--`
   * سریع/پایدار: `workers 50--`

2. **افزایش تایم‌اوت برای شبکه‌های کند**:

   ```bash
   python test_doh.py example.com --timeout 15
   ```

3. **کاهش تلاش‌ها برای بررسی سریع**:
...
1. **استفاده از تلاش‌های بیشتر برای اعتبارسنجی محیط تولید**:
